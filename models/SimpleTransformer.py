...
class SimpleTransformer(torch.nn.Module):
    ...
    #                     predicted token IDs, padded to the maximum length.
    #     """
    #     # Set a default maximum length if not provided.
    #     if max_len is None or max_len <= 0:
    #         max_len = int(src.size(1) * 1.6) + 10
    #
    #     with torch.no_grad():
    ...
    #         src_padding_mask = self._generate_padding_mask(src, self.pad_token_id)
    #         src_embed = self.dropout_enc(self.positional_encoding_encoder(
    #                     self.embedding_encoder(src) * self.embed_scale))
    ...
    #         # Repeat the encoder output for each beam to enable parallel decoding.
    #         # Original shape: (batch_size, src_seq_len, embedding_dim)
    #         # Repeated shape: (batch_size, beam_size, src_seq_len, embedding_dim)
    ...
    #         # --- Decoding loop ---
    #         for _ in range(max_len):
    #             trg_embed = self.dropout_dec(self.positional_encoding_decoder(
    ...
    #
    #             # Expand beams and calculate score
