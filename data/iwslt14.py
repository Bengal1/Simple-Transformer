...
class IWSLT14Dataset(Dataset):
    ...
    def __init__(self, split="train", local_file=None):
        """Initializes the dataset for the specified split (train, validation, test)."""
...
#
#     def _load_dataset(self):
#         """Loads the IWSLT14 dataset and tokenizes sentences for train, validation, and test."""
#         dataset = load_dataset("ahazeemi/iwslt14-en-fr")
#
#         self.tokenized_data = {}
#
#         for split in ["train", "validation", "test"]:
#             tokenized_en = [self._tokenize_text(sentence, self.en_nlp) for sentence in dataset[split]["en"]]
#             tokenized_fr = [self._tokenize_text(sentence, self.fr_nlp) for sentence in dataset[split]["fr"]]
#             self.tokenized_data[split] = (tokenized_en, tokenized_fr)
#
#     @staticmethod
#     def _tokenize_text(text: str, nlp_model) -> list:
#         """Tokenizes a given text using the specified spaCy NLP model.
