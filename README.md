...
According to [*Johnson–Lindenstrauss lemma*](https://en.wikipedia.org/wiki/Johnson%E2%80%93Lindenstrauss_lemma) if we "cram" more vectors in the space and ease the rigid demand of [*Orthogonality*](https://en.wikipedia.org/wiki/Orthogonality) a little bit and allow a slight deviation, $`0<ε<1`$, meaning we can arrange the vetors, not in exactly 90° between each other, but between the range of $`90°-ε \le x \le 90°+ε`$, each vector will have an angle of $`[90°-ε , 90°+ε]`$ with all other vectors. Then we could arrage D vecotrs in $`ℝ^{E}`$, when *D ≈* **O**$`\bigg( exp(E·ε^2) \bigg) `$.<br/>
