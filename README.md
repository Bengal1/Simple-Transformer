...
According to [*Johnson–Lindenstrauss lemma*](https://en.wikipedia.org/wiki/Johnson%E2%80%93Lindenstrauss_lemma) if we compress the vectors a bit in the space, with a slight deviation, $`0<ε<1`$, meaning not exactly 90° degrees, instead of being orthogonal the will be an angle of $`[90°-ε , 90°+ε]`$ betwen all vectors, we could arrage D vecotrs in $`ℝ^{E}`$, when *D ≈* **O**$`\bigg( exp(E·ε^2) \bigg)`$, then for exapmle in $`ℝ^{100}`$ we can arrange ~exp(100) ≈ $`2.68·10^{43}`$ vectors/directions/semantics and that is a lot of semantics!
