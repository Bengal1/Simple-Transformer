...
<img align="right" width="500" height="550" src="https://github.com/user-attachments/assets/c99a2410-d73c-451d-ae9b-6b4bd606e45d">
...
There is a hypothesis that tries to explain this. according to [*Johnson–Lindenstrauss lemma*](https://en.wikipedia.org/wiki/Johnson%E2%80%93Lindenstrauss_lemma) ןf we compress the vectors a bit in space, with a slight deviation, *ε*, meaning not exactly 90 degrees, instead of being orthogonal the will be an angle of $`[90-ε , 90+ε]`$ betwen all vectors, we could arrage D vecotrs in $`ℝ^{E}`$, when  $`D ≈ \Omega \bigg(exp(E·ε^2) \bigg)`$, then for exapmle in $`ℝ^{100}`$ and for $`ε=1°`$ we can arrange ~exp(100) ≈ $`2.68·10^43`$ and that is a lot!
